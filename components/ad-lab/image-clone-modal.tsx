...
import type { BrandBrain, ImageCloneLine, MetaAdResult, VisualDirection } from "@/lib/types"
import {
  startImageClone,
...
// 3 = Dirección Visual (nuevo — un art director en Claude decide qué
// conservar/traducir/evitar ANTES de generar imagen). 4 = configuración +
// generación (era el "step 3" original).
type Step = 1 | 2 | 3 | 4
...
export function ImageCloneModal({ ad, recloneSource, savedAdSource, onClose }: Props) {
  ...
  // Paso 3 — Dirección Visual: un plan de art direction (Claude) que se
  // genera automáticamente al entrar al paso, y que el usuario puede
  // ajustar/regenerar antes de continuar a la generación de imagen.
  ...
  const [directionAdjustments, setDirectionAdjustments] = useState("")
  ...
  const runGenerateDirection = useCallback((extra?: string) => {
    if (!cloneId) return
    setIsGeneratingDirection(true)
    setDirectionError(null)
    generateVisualDirection(cloneId, extra)
      .then((direction) => { setVisualDirection(direction); setDirectionAdjustments("") })
      .catch((err) => setDirectionError(String(err)))
      .finally(() => setIsGeneratingDirection(false))
  }, [cloneId])

  // Se dispara solo al entrar al paso 3 la primera vez — regenerar
  // (con o sin ajustes) es siempre una acción explícita del usuario
  // desde ahí en adelante, nunca automática de nuevo.
  useEffect(() => {
    if (step === 3 && cloneId && !visualDirection && !isGeneratingDirection && !directionError) {
      runGenerateDirection()
    }
  }, [step, cloneId, visualDirection, isGeneratingDirection, directionError, runGenerateDirection])
      ...
      <div className="relative w-full max-w-4xl bg-background rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 mx-auto">
            {recloneSource ? (
              // 3-dot indicator: step 1 (elegir marca) → step 3 (dirección
              // visual, nueva) → step 4 (generar). El paso 2 (editar copy)
              // se salta aquí — este flujo hereda el copy del clon padre.
              ([{ val: 1 as Step, label: 1 }, { val: 3 as Step, label: 2 }, { val: 4 as Step, label: 3 }]).map(({ val, label }, i) => (
                <div key={val} className="flex items-center gap-1.5">
                  ...
                  {i < 2 && <div className={`w-8 h-px transition-colors ${step > val ? "bg-violet-300" : "bg-border"}`} />}
                </div>
              ))
            ) : (
              ([1, 2, 3, 4] as Step[]).map((s) => (
                <div key={s} className="flex items-center gap-1.5">
                  ...
                  {s < 4 && <div className={`w-8 h-px transition-colors ${step > s ? "bg-violet-300" : "bg-border"}`} />}
                </div>
              ))
            )}
          </div>
        ...
        {step === 3 && (
          <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div>
                <p className="text-sm font-medium">Dirección visual</p>
                <p className="text-xs text-muted-foreground">
                  Un art director (IA) analizó el anuncio y define qué conservar, qué traducir a tu marca y qué evitar — esto guía la generación de imagen del siguiente paso.
                </p>
              </div>
              ...
              {isGeneratingDirection && (
                <div className="flex items-center gap-3 bg-violet-50 rounded-xl px-4 py-3 text-sm text-violet-700">
                  ...
                  Analizando el anuncio y definiendo la dirección visual…
                </div>
              )}

              {directionError && !isGeneratingDirection && (
              ...
              {visualDirection && !isGeneratingDirection && (
                <div className="space-y-3">
                  <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-4">
                    <p className="text-sm font-semibold text-violet-800">{visualDirection.direction_name}</p>
                    <p className="text-xs text-muted-foreground mt-1">{visualDirection.summary}</p>
                  </div>

                  <div className="grid sm:grid-cols-2 gap-3">
                    <div className="rounded-xl border border-border p-3">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-emerald-700 mb-1.5">Conservar</p>
                      <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                        {visualDirection.design_diagnosis.retain.map((r, i) => <li key={i}>{r}</li>)}
                      </ul>
                    </div>
                    <div className="rounded-xl border border-border p-3">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-sky-700 mb-1.5">Traducir</p>
                      <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                        {visualDirection.design_diagnosis.translate.map((t, i) => <li key={i}>{t}</li>)}
                      </ul>
                    </div>
                  </div>

                  <div className="rounded-xl border border-border p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Ancla visual</p>
                    <p className="text-xs">{visualDirection.visual_anchor.source} — <span className="text-muted-foreground">{visualDirection.visual_anchor.reason}</span></p>
                  </div>

                  <div className="rounded-xl border border-border p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Estilo</p>
                    <p className="text-xs">{visualDirection.style.keywords.join(", ")} · intensidad: <span className="font-medium">{visualDirection.style.intensity}</span></p>
                  </div>

                  <div className="rounded-xl border border-border p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Roles de color</p>
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(visualDirection.palette).filter(([, v]) => v?.trim()).map(([role, hex]) => (
                        <div key={role} className="flex items-center gap-1.5 text-xs bg-muted/50 rounded-full px-2 py-1">
                          <span className="w-3 h-3 rounded-full border border-black/10 flex-shrink-0" style={{ background: /^#/.test(hex) ? hex : undefined }} />
                          <span className="text-muted-foreground">{role.replace(/_/g, " ")}:</span> {hex}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-border p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Fondo</p>
                    <p className="text-xs">
                      {visualDirection.background_strategy.preserve_literal_environment ? "Conservar el ambiente original" : "Traducir el ambiente"} — <span className="text-muted-foreground">{visualDirection.background_strategy.destination_treatment}</span>
                    </p>
                  </div>

                  {visualDirection.avoid.length > 0 && (
                    <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-3">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-destructive mb-1.5">Evitar</p>
                      <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                        {visualDirection.avoid.map((a, i) => <li key={i}>{a}</li>)}
                      </ul>
                    </div>
                  )}

                  <div className="rounded-xl border border-border p-3 space-y-2">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">¿Quieres ajustar algo?</p>
                    <textarea
                      value={directionAdjustments}
                      onChange={(e) => setDirectionAdjustments(e.target.value)}
                      placeholder="Ej. usa más saturación, no conserves el fondo original, dale más énfasis al CTA…"
                      rows={2}
                      className="w-full text-sm rounded-lg border border-input bg-background px-3 py-2 resize-none outline-none focus:ring-2 focus:ring-ring"
                    />
                    <button
                      onClick={() => runGenerateDirection(directionAdjustments.trim() || undefined)}
                    ...
                    >
                      <Compass className="w-3.5 h-3.5" /> Regenerar dirección visual
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              {isGenerating && (
                <div className="flex items-center gap-3 bg-violet-50 rounded-xl px-4 py-3 text-sm text-violet-700">
                  <Loader2 className="w-4 h-4 animate-spin flex-shrink-0" />
                  Generando imágenes… puede tomar 30–90 s.
                </div>
              )}
            </div>
          </div>
        ...
        {step === 4 && generatedUrls.length > 0 && !isFinalized && (
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {acceptedUrls.length > 0 && (
              <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 rounded-xl px-3 py-2 text-xs font-medium">
                <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0" />
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border flex-shrink-0">
          ...
          {step === 3 && (
            <button
              onClick={() => setStep(4)}
              disabled={!visualDirection || isGeneratingDirection}
              className="inline-flex items-center gap-2 h-9 px-5 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-50 transition-colors"
            >
              Continuar <ChevronRight className="w-4 h-4" />
            </button>
