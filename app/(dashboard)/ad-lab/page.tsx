  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard icon={ImagePlay}  label="Creativos generados" value={creativesCount} href="/ad-lab/creatives"  />
      </div>

      {/* Nav cards — 4 columns */}
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <NavCard href="/ad-lab/discovery"  icon={LayoutGrid} title="Discovery"  description="Busca anuncios en Meta Ads Library por marca o competidor." />
...
function StatCard({ icon: Icon, label, value, href }: { icon: React.ElementType; label: string; value: number; href: string }) {
  return (
    <a href={href} className="group rounded-xl border border-border bg-card px-5 py-4 flex items-center gap-4 hover:border-primary/40 hover:bg-primary/5 transition-colors">
...
function NavCard({ href, icon: Icon, title, description }: { href: string; icon: React.ElementType; title: string; description: string }) {
  return (
    <a href={href} className="group rounded-xl border border-border bg-card p-5 flex flex-col gap-3 hover:border-primary/40 hover:shadow-sm transition-all">
